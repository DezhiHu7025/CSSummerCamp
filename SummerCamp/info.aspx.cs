...
namespace SummerCamp
{
    ...
    {
        string strCon = "Data Source=172.27.100.247;Database=db_forminf;Uid=sa;Pwd=QQpp@1100";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                this.txt_country.Text = "（非必填）";
                this.txt_add.Text = "（必填）";
                //this.txt_gread.Text = "（必填）";
                this.drp_gread.SelectedValue = "-99";
        ...
        protected void GetTaskContent(int ida)
        {
            ...
            string sqlstr = "SELECT * FROM [db_forminf].[dbo].[product] where shop_id='S0000048' and  (Is_inner ='Z' OR Is_inner = 'N') AND Is_open = 'Y'  and id =" + ida;
 ...
 on a.merchantSeq=b.merchantSeq  where a.id is not null  and a.refundtime is null and  a.tradestatus='S' and b.remark = '" + this.HiddenField7.Value.Trim() + "'";
            ...
            if (myViewDate1.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    //如果报名人数已满，更新关闭课程
                    string sqlstr2 = "update  [db_forminf].[dbo].[product] set Is_open = 'N' where shop_id='S0000048' and  product_id = '" + this.HiddenField7.Value.Trim() + "'";
        ...
        protected void baoming_Click(object sender, EventArgs e)
        {
            ...
            //    return;
            //}
            if (Convert.ToInt32(this.drp_gread.SelectedValue) == -99)
            {
                Response.Write("<Script Language=JavaScript>alert('新增失败！请选择学生现读年级！，若有特殊情况请咨询学校招生办！');</Script>");
                return;
            }
...
values(@addtime,@mchSeqNo,@amount,@orderInfo,@remark,@inExtData)";
            ...
            SqlParameter p4 = new SqlParameter("@orderInfo", "2024幼儿园夏令营");
